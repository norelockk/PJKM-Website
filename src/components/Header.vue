<template>
	<div class="my-0 mx-0" style="height: fit-contents; background-color: black;">
		<v-row class="ma-0">
			<v-col cols="1" class="ma-0 pa-0">
				<router-link to="/">
					<v-img src="src/assets/logos/PJKT-03.png" height="75" class="pa-0 ma-0" />
				</router-link>
			</v-col>
			<v-col cols="4" class="ma-0 pa-0">
				<v-row class="mx-0 mb-0 mt-7 py-0">
					<v-col cols="2" v-for="link in links.navlinks" class="text-right mono text-h6 my-0 py-0"
						style="color:white">
						<v-menu transition="slide-y=transition" openOnHover v-if="link.subLocs">
							<template v-slot:activator="{ props }">
								<div v-bind="props">
									{{ link.name }}
									<svg version="1.1" viewBox="0 0 8 5" xmlns="http://www.w3.org/2000/svg"
										xmlns:xlink="http://www.w3.org/1999/xlink" width="8px" height="5px"
										class="chevron" v-if="link.subLocs"
										:class="props['aria-expanded'] == 'true' ? 'chevron--flip' : ''">
							...
							<v-list>
								<v-list-item v-for="(sublink, i) in link.subLocs" :key="i">
									<v-list-item-title>
						...
						<router-link :to="link.url" v-else class="mr-2">{{ link.name }}</router-link>
					</v-col>
				</v-row>
			</v-col>
			<v-col cols="1"></v-col>
			...
			<v-col cols="2">
				<v-row justify="end" class="mr-5">
					<v-col cols="1" v-for="(social) in links.socials" class="mr-5">
						<v-icon icon="fa-discord"></v-icon>
						<a :href="social.url" target="_blank">
							<FontAwesomeIcon :icon="'fa-brands ' + social.logo" inverse size="xl" />
						</a>
					</v-col>
				</v-row>
			</v-col>
...
let links = {
	navlinks: {
		events: {
			...
			subLocs: {
				first: {
					name: "Projekt: Community 2023",
					url: '/events/pjktcom2023'
				}
			}
		},
		new: {
	...
	socials: {
		discord: {
			logo: "fa-discord",
			url: "https://discord.gg/PJKT"
		},
		youtube: {
			logo: "fa-youtube",
			url: "https://youtube.com/PJKTCOM"
		},
		twitter: {
			logo: "fa-twitter",
			url: "https://twitter.com/PJKTCOM"
		},
